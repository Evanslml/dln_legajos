  ...
  function AddCertificado(parameter) {
      return '<div class="col-md-3 border-seccion">\
                <h6 class="subtitle" style="margin:9px">TIPO <i class="danger">*</i></h6>\
                  ...
                  <select data-placeholder="SELECCIONE" class="chosen-select-deselect cbx_grado1" tabindex="2" name="cbx_tipo_licencia" id="cbx_tipo_licencia">\
                      <option value=""></option>\
...
/*********************************************************************************************************************************/
  ...
  function get_data_form() {
    ...
    for (var i = 1; i <= nFilas_childs; i++) {
          var MTABL_ID          = $(".datos_certificados .row-agregado-resolucion:nth-child("+i+") #cbx_tipo_licencia").find('option:selected').prop('value');
          var MDOC_DESCRIPCION  = $('.datos_certificados .row-agregado-resolucion:nth-child('+i+') input#txt_num_resolucion').val().toUpperCase();
          var MDOC_ALIAS        = $(".datos_certificados .row-agregado-resolucion:nth-child("+i+") input#txt_motivo").val().toUpperCase();
          if(MTABL_ID.length == 0){
              var mensaje = 'Debe Ingresar el número de resolución'; swal_mensaje_error(mensaje); return false;
          }else if(MDOC_DESCRIPCION.length == 0){
              var mensaje = 'Debe Ingresar el motivo'; swal_mensaje_error(mensaje); return false;
          }
          array.push(MTABL_ID,MDOC_DESCRIPCION,MDOC_ALIAS);
    }
