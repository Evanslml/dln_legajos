  ...
  function AddCertificado(parameter) {
      return '<div class="col-md-4 border-seccion">\
                <h6 class="subtitle" style="margin:9px">TIPO <i class="danger">*</i></h6>\
                  ...
                  <select data-placeholder="SELECCIONE" class="chosen-select-deselect cbx_grado1" tabindex="2" name="cbx_demeritos" id="cbx_demeritos">\
                      <option value=""></option>\
                      <option value="75">RES. DE AMONESTACIONES</option><option value="76">RES. DE SUSPENSIONES</option><option value="77">RES. DE CESE TEMPORAL</option><option value="78">RES. DE PROC. ADMIN. Y RESP. PECUANIARIA</option><option value="79">RES. DE DESTITUCION</option>\
                  </select>\
            </div>\
...
/*********************************************************************************************************************************/
  ...
  function get_data_form() {
    ...
    for (var i = 1; i <= nFilas_childs; i++) {
          var MTABL_ID          = $(".datos_certificados .row-agregado-resolucion:nth-child("+i+") #cbx_demeritos").find('option:selected').prop('value');
    ...
    //console.log(array);
      ...
      $.ajax({
          type: 'POST',
          url: './public/user/ajax/secciones/seccionxiv.php?action=formulario',
...
function subida_realizada(i){
  ...
  for (var m = 1; m <= nFilas_childs; m++) {
      ...
      archivos.push({dni:MPERS_NUMDOC, obj:MOBJ_ID, nom:NOMBRE, arc: '38'+'.'+i, fil: input  });
  }  
