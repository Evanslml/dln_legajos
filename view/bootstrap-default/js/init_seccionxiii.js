  ...
  function AddCertificado(parameter) {
      return '<div class="col-md-4 border-seccion">\
                <h6 class="subtitle" style="margin:9px">TIPO <i class="danger">*</i></h6>\
                  ...
                  <select data-placeholder="SELECCIONE" class="chosen-select-deselect cbx_grado1" tabindex="2" name="cbx_tipo_merito" id="cbx_tipo_merito">\
                      <option value=""></option>\
                      <option value="72">RES. DE FELICITACION</option><option value="73">RES. DE AGRADECIMIENTO</option>\
                  </select>\
            </div>\
            <div class="col-md-4 border-seccion formulario-legajos">\
                <div class="group">\
                  ...
                  <span class="bar"></span>\
                  <label>Nº DE RESOLUCIÓN <i class="danger">*</i></label>\
                </div>\
            </div>\
  ...
  function AddCertificado(parameter) {
      return '<div class="col-md-4 border-seccion">\
                <h6 class="subtitle" style="margin:9px">TIPO <i class="danger">*</i></h6>\
                  ...
                  <select data-placeholder="SELECCIONE" class="chosen-select-deselect cbx_grado1" tabindex="2" name="cbx_tipo_merito" id="cbx_tipo_merito">\
                      <option value=""></option>\
                      <option value="68">FICHA DE EVALUACION</option><option value="69">INFORME</option><option value="70">RESULTADO DE EVALUACION</option>\
                  </select>\
            </div>\
            <div class="col-md-4 border-seccion formulario-legajos">\
                <div class="group">\
...
/*********************************************************************************************************************************/
  ...
  function get_data_form() {
    ...
    for (var i = 1; i <= nFilas_childs; i++) {
          var MTABL_ID          = $(".datos_certificados .row-agregado-resolucion:nth-child("+i+") #cbx_tipo_merito").find('option:selected').prop('value');
    ...
    //console.log(array);
      ...
      $.ajax({
          type: 'POST',
          url: './public/user/ajax/secciones/seccionxiii.php?action=formulario',
...
function subida_realizada(i){
  ...
  for (var m = 1; m <= nFilas_childs; m++) {
      ...
      archivos.push({dni:MPERS_NUMDOC, obj:MOBJ_ID, nom:NOMBRE, arc: '37'+'.'+i, fil: input  });
  }  
