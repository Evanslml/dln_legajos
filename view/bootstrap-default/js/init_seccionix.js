  ...
  function AddCertificado(parameter) {
      return '<div class="col-md-4 border-seccion">\
                <h6 class="subtitle" style="margin:9px">TIPO <i class="danger">*</i></h6>\
                  ...
                  <select data-placeholder="SELECCIONE" class="chosen-select-deselect cbx_grado1" tabindex="2" name="cbx_tipo_ascenso" id="cbx_tipo_ascenso">\
                      <option value=""></option>\
                      <option value="63">RES. DE ASCENSOS</option><option value="64">RES. DE RECATEGORIZACION</option><option value="65">RES. DE ADECUACIÓN</option><option value="66">RES. DE PROMOCION</option>\
                  </select>\
            </div>\
...
/*********************************************************************************************************************************/
  ...
  function get_data_form() {
    ...
    for (var i = 1; i <= nFilas_childs; i++) {
          var MTABL_ID          = $(".datos_certificados .row-agregado-resolucion:nth-child("+i+") #cbx_tipo_ascenso").find('option:selected').prop('value');
    ...
    //console.log(array);
      ...
      $.ajax({
          type: 'POST',
          url: './public/user/ajax/secciones/seccionix.php?action=formulario',
...
function subida_realizada(i){
  ...
  for (var m = 1; m <= nFilas_childs; m++) {
      ...
      archivos.push({dni:MPERS_NUMDOC, obj:MOBJ_ID, nom:NOMBRE, arc: '32'+'.'+i, fil: input  });
  }  
